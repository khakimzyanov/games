(function() {
    /* Хранит состояние и настройки игры  */
    const TETRIS = {
        delays: [600,500,400,350,300,250,200,150,120], // варианты задержек перед смещением вниз
        scoreStep: 1000, // шаг очков для увеличения скорости (уменьшения задержки)
        score: 0, // текущее количество очков
        speed: 0, // скорость (индекс для выбора из массива this.delays)
    ...
    /* Наблюдатель за количеством очков (для увеличения скорости). */
    new MutationObserver(x => {
        let score = Number (x[0].target.innerText);
        TETRIS.speed = Math.floor(score / TETRIS.scoreStep);
        if (!TETRIS.delays[TETRIS.speed]) {
            TETRIS.speed = TETRIS.delays.length - 1;
        }
        document.getElementById('speed').innerText = TETRIS.speed + 1;		
    ...
    function start() {
        ...
        TETRIS.timerID = setTimeout(start, TETRIS.delays[TETRIS.speed]);
    }
    ...
    function clean() {	
        ...
        for (let i = 0; i < TETRIS.vcells.length; i++) {
            if (!TETRIS.vcells[i].includes(0)) {
                rows.push(i);
                TETRIS.rcells[i].forEach(x => x.style.backgroundColor = '#272822');
        ...
        setTimeout(function() {
            for (let row of rows) {
                TETRIS.rcells[row].forEach(x => x.style.backgroundColor = '');
            }
            render();
        }, 200);
        if (rows.length && TETRIS.score <= 1000000000) {
            document.getElementById('score').innerText
                = TETRIS.score += [,100, 300, 700, 1500][rows.length];
        }
    }
